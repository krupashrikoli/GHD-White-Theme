...
import {
  hasResumableCheckoutModal,
  loadPersistedCheckout,
} from "../components/booking/bookingCheckoutPersistence";

function bookingResumeSearch() {
  ...
  return {
    hotelId: p.selectedHotelId || "nivaara-nerul",
    checkIn: p.search.checkIn,
    checkOut: p.search.checkOut,
    adults: String(p.search.adults),
    children: String(p.search.children),
  };
}
...
export function PoliciesPage() {
  ...
  const sectionTitle =
    "font-body text-sm font-semibold uppercase tracking-[0.2em] text-charcoal border-b border-gold/30 pb-2 mb-4";
  const subTitle =
    "font-body text-xs font-semibold uppercase tracking-[0.18em] text-charcoal/75 mt-6 mb-2";
  const p =
    "font-body text-sm sm:text-[0.95rem] text-charcoal/80 leading-relaxed";
  const li =
    "font-body text-sm sm:text-[0.95rem] text-charcoal/80 leading-relaxed";
    ...
    <div className="min-h-screen flex flex-col relative bg-cream">
      ...
      <section className="relative z-10 flex-1 section-pad pt-28 sm:pt-32">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-10">
          ...
          {hasResumableCheckoutModal() ? (
            <div className="mb-10 rounded-xl border border-gold/35 bg-white/90 px-4 py-3 text-center shadow-md shadow-black/10 sm:px-5">
              <p className="text-sm text-charcoal/80 mb-2">
                You have a booking in progress. Your details are saved in this
                tab until you finish or leave the booking page.
              </p>
              <Link
                to="/booking"
                search={bookingResumeSearch()}
                className="inline-flex text-sm font-semibold text-gold underline underline-offset-4 hover:text-charcoal transition"
              >
                Return to checkout
              </Link>
            </div>
