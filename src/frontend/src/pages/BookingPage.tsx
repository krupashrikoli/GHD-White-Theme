...
function restoredCheckoutSelection(): BookingRateSelection | null {
  const p = loadPersistedCheckout();
  const sel = p?.selection;
  if (!sel?.rooms?.some((r) => r.quantity > 0)) return null;
  return sel;
}
...
export function BookingPage() {
  const [checkoutOpen, setCheckoutOpen] = useState(() => {
    const p = loadPersistedCheckout();
    const sel = restoredCheckoutSelection();
    return Boolean(p?.modalOpen && sel);
  });
  const [checkoutSelection, setCheckoutSelection] =
    useState<BookingRateSelection | null>(() => restoredCheckoutSelection());
  const [selectedHotelId, setSelectedHotelId] = useState(() =>
    loadPersistedCheckout()?.selectedHotelId ?? "nivaara-nerul",
  );
  const [liveSearch, setLiveSearch] = useState<BookingSearchSnapshot>(() => {
    ...
    if (p?.search) {
      return {
        ...
        rooms: p.search.rooms,
      };
    }
    return readBookingSearch();
  });
  const [meals, setMeals] = useState<MealSelection>(() => {
    const m = loadPersistedCheckout()?.meals;
    return m ?? defaultMeals();
  });
  const [roomAssignments, setRoomAssignments] = useState<RoomCategoryId[]>(() => {
    const ra = loadPersistedCheckout()?.roomAssignments;
    ...
    <div className="bg-cream min-h-screen flex flex-col">
      <section className="flex-1 section-pad pt-28 sm:pt-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
          ...
          <div className="mx-auto mb-12 w-full max-w-4xl rounded-xl border border-gold/25 bg-white/95 px-3 py-3 shadow-lg shadow-black/15 sm:px-4 sm:py-4">
            <HomeSearchBar
      ...
      {checkoutOpen && checkoutSelection ? (
        <BookingCheckoutModal
          ...
          search={liveSearch}
          selection={checkoutSelection}
          initialFormDraft={loadPersistedCheckout()?.form ?? null}
        />
      ) : null}
      <RoomDetailsModal
