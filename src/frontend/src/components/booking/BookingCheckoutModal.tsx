...
import {
  ...
  defaultCheckoutFormDraft,
  mergePersistedCheckout,
  type CheckoutFormDraft,
} from "./bookingCheckoutPersistence";
import {
  BOOKING_TAX_RATE,
...
function mergeFormInitial(
  initial: CheckoutFormDraft | null | undefined,
): CheckoutFormDraft {
  return { ...defaultCheckoutFormDraft(), ...(initial ?? {}) };
}
...
export function BookingCheckoutModal(props: {
  ...
  /** Restored from session when returning to checkout (same tab). */
  initialFormDraft?: CheckoutFormDraft | null;
}) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  const titleId = useId();
  const [firstName, setFirstName] = useState(
    () => mergeFormInitial(props.initialFormDraft).firstName,
  );
  const [surname, setSurname] = useState(
    () => mergeFormInitial(props.initialFormDraft).surname,
  );
  const [phone, setPhone] = useState(
    () => mergeFormInitial(props.initialFormDraft).phone,
  );
  const [email, setEmail] = useState(
    () => mergeFormInitial(props.initialFormDraft).email,
  );
  const [country, setCountry] = useState(
    () => mergeFormInitial(props.initialFormDraft).country,
  );
  const [address1, setAddress1] = useState(
    () => mergeFormInitial(props.initialFormDraft).address1,
  );
  const [address2, setAddress2] = useState(
    () => mergeFormInitial(props.initialFormDraft).address2,
  );
  const [city, setCity] = useState(
    () => mergeFormInitial(props.initialFormDraft).city,
  );
  const [stateProvince, setStateProvince] = useState(
    () => mergeFormInitial(props.initialFormDraft).stateProvince,
  );
  const [createAccount, setCreateAccount] = useState(
    () => mergeFormInitial(props.initialFormDraft).createAccount,
  );
  const [cardNumber, setCardNumber] = useState(
    () => mergeFormInitial(props.initialFormDraft).cardNumber,
  );
  const [cardExp, setCardExp] = useState(
    () => mergeFormInitial(props.initialFormDraft).cardExp,
  );
  const [cardCvv, setCardCvv] = useState(
    () => mergeFormInitial(props.initialFormDraft).cardCvv,
  );
  const [nameOnCard, setNameOnCard] = useState(
    () => mergeFormInitial(props.initialFormDraft).nameOnCard,
  );
  const [agreePrivacy, setAgreePrivacy] = useState(
    () => mergeFormInitial(props.initialFormDraft).agreePrivacy,
  );
  const [submitError, setSubmitError] = useState("");
  const [couponInput, setCouponInput] = useState(
    () => mergeFormInitial(props.initialFormDraft).couponInput,
  );
  const [couponError, setCouponError] = useState("");
  /** Normalized code from last successful apply; discount is recomputed from current subtotal. */
  const [appliedCouponCode, setAppliedCouponCode] = useState<string | null>(
    () => mergeFormInitial(props.initialFormDraft).appliedCouponCode,
  );
  ...
  const pricingBase = useMemo(() => {
    ...
    const taxableBeforeCoupon = roomSub + surcharges;
    return {
      roomSub,
      roomBaseSub,
      mealsSub,
  ...
  const pricing = useMemo(() => {
    const b = pricingBase;
    const couponParse = appliedCouponCode
      ? parseBookingCoupon(appliedCouponCode, b.taxableBeforeCoupon)
      : null;
    const discount =
      couponParse && couponParse.ok
        ? Math.min(couponParse.discountInr, b.taxableBeforeCoupon)
        : 0;
    const taxableAfterCoupon = Math.max(0, b.taxableBeforeCoupon - discount);
    const taxes =
      Math.round(taxableAfterCoupon * BOOKING_TAX_RATE * 100) / 100;
    const total = Math.round((taxableAfterCoupon + taxes) * 100) / 100;
    return {
      ...b,
      discount,
      taxableAfterCoupon,
  ...
  const formDraftSnapshot = useMemo(
    (): CheckoutFormDraft => ({
      firstName,
      surname,
      phone,
  ...
  useEffect(() => {
    mergePersistedCheckout({ form: formDraftSnapshot });
  }, [formDraftSnapshot]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const couponLine = pricing.appliedCouponCode
      ? `\nCoupon: ${pricing.appliedCouponCode} (−${formatInr(pricing.discount)})`
      : "";
    // eslint-disable-next-line no-alert
    alert(
      `Booking request recorded (demo).\nTotal: ${formatInr(pricing.total)}${couponLine}\nConfirmation would be sent to ${email.trim()}.`,
    );
    clearPersistedCheckout();
    props.onClose();
  };
  ...
  const handleApplyCoupon = () => {
    setCouponError("");
    const r = parseBookingCoupon(
      couponInput,
      pricingBase.taxableBeforeCoupon,
    );
    if (!r.ok) {
      setCouponError(r.error);
      return;
    }
    setAppliedCouponCode(r.code);
      ...
      <div className="flex max-h-[calc(100vh-2rem)] w-full flex-col overflow-hidden rounded-2xl">
        <div className="flex flex-shrink-0 items-center justify-between border-b border-charcoal/10 bg-[#faf9f7]/95 px-5 py-4 backdrop-blur-sm sm:px-8">
          ...
          <button
            ...
            className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border border-charcoal/15 text-charcoal/70 transition hover:bg-cream/5 hover:text-charcoal"
            aria-label="Close"
          >
          ...
          <div className="grid gap-8 lg:grid-cols-[1fr_minmax(280px,340px)]">
            ...
            <div className="order-2 min-w-0 space-y-8 lg:order-1">
              ...
              <section className="rounded-xl border border-charcoal/10 bg-white/80 p-4 sm:p-5">
                ...
                <Link
                  to="/policies"
                  className="mt-4 inline-block text-sm font-medium text-gold underline underline-offset-4 hover:text-charcoal transition"
                >
                  View Full Policy
                </Link>
              </section>
            ...
            <aside className="order-1 lg:order-2 lg:sticky lg:top-24 h-fit">
              <div className="rounded-xl border border-gold/25 bg-white p-5 shadow-lg shadow-black/10">
                ...
                <div className="my-4 border-t border-charcoal/10 pt-4 space-y-3">
                  <div>
                    <label
                      htmlFor="bk-coupon"
                      className={`${labelClass} normal-case tracking-normal text-xs`}
                    >
                      Coupon code
                    </label>
                    <div className="mt-1 flex gap-2">
                      <input
                        ...
                        autoComplete="off"
                        disabled={Boolean(pricing.appliedCouponCode)}
                      />
                      {pricing.appliedCouponCode ? (
                        <button
                          type="button"
                          onClick={handleRemoveCoupon}
                          className="flex-shrink-0 rounded-lg border border-charcoal/20 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-charcoal transition hover:bg-charcoal/5"
                        >
                          Remove
                        </button>
                      ) : (
                        <button
                          ...
                          className="flex-shrink-0 rounded-lg border border-gold/50 bg-gold/15 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-charcoal transition hover:bg-gold/25"
                        >
                          Apply
                        </button>
                      )}
